...
import { useSeason } from "@/components/ThreeDPortfolio/SeasonProvider";
import * as THREE from "three";
import { SKILLS_GRID, type SkillIcon } from "@/lib/threeDPortfolio/skills";
...
const SECTION_STATES: Record<string, KeyboardState> = {
  ...
  stack: {
    // Centered, moderate isometric tilt — enough yaw to show the left face
    // but not so much pitch that the base wall takes over the silhouette.
    // posY kept close to 0 (rather than the source's -0.6) so the model
    // sits high enough in the camera frame to stay close to the heading
    // above it instead of leaving a dead gap.
    yaw: Math.PI * 0.40,
    pitch: Math.PI * 0.14,
    roll: Math.PI * 0.13,
    posX: 0,
...
const KEY_SOUND_URLS = [
  "/3d-portfolio/sounds/switch_press.mp3",
  "/3d-portfolio/sounds/switch_release.mp3",
] as const;
...
// Eagerly create the AudioContext + start loading the click samples as soon
// as the keyboard mounts, instead of waiting for the first hover. Hover
// alone can't unlock Web Audio (only click/keydown/touchstart count as a
// user gesture), so warming this up early means the unlock listener is
// already armed by the time the user's first real click/tap happens
// anywhere on the page — well before they scroll down to hover a key.
function warmAudio(): AudioContext | null {
  ...
  try {
    ...
    if (audioCtx.state !== "closed") loadKeySounds(audioCtx);
    return audioCtx;
  } catch {
    return null;
  }
...
function Keycap({
  ...
  // Sampled once at mount — empty deps make the memo cache (not re-evaluate)
  // even under StrictMode's double-render, so the Math.random() calls below
  // are safe despite the purity rule's generic warning.
...
function Keyboard({
  ...
  mobile: boolean;
  onHoveredIconChange?: (icon: SkillIcon | null) => void;
}) {
  ...
  // Warm the AudioContext + preload click samples as soon as the keyboard
  // mounts (see warmAudio's comment) instead of waiting for the first hover.
  useEffect(() => {
    warmAudio();
  }, []);
  ...
  // Report the hovered icon (only meaningful on the stack section, desktop
  // only) up to the parent so it can render a normal DOM callout instead of
  // in-canvas 3D text.
  useEffect(() => {
    onHoveredIconChange?.(
      !mobile && activeSection === "stack" ? hoveredIcon : null
    );
  }, [hoveredIcon, activeSection, mobile, onHoveredIconChange]);
...
export default function FrozenKeyboard({
  ...
  return (
    ...
    >
      ...
      <Keyboard mobile={mobile} onHoveredIconChange={onHoveredIconChange} />
