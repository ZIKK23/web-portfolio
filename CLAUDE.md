...
- `components/` — one folder per page (`Landing/` — `HeroIntro.tsx`, `RolePicker.tsx`, see below; `Projects/`, `About/`, `Contact/`) plus shared `Nav.tsx`, `Footer.tsx`; `components/animations/<source-name>/` for ported GitHub animation effects (e.g. `3d-sticky-cards/`).
- `lib/` — typed data, e.g. `projects.ts` (Projects page sticky-card content).
